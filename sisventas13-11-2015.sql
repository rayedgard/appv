...
MySQL - 5.6.26-log : Database - sisventas
*********************************************************************
*/
...
insert  into `cliente`(`documento`,`razon`,`telefono`,`direccion`,`fechaRegistro`,`email`,`estado`) values ('','-','-','-','2015-11-13','-',0),('10404942501','EDGARD RAYME','','AV. LA CULTURA 772','2015-09-02','',0),('10494252000','INFORMATIC TECHNOLOGY DEVELOPMENT CORPORATION S.A.','','AV LA CULTURA 772 INT 205','2015-09-05','',0),('20490253867','SDASD','','DSA','2015-09-18','',0);
...
insert  into `descuento`(`id`,`criterio`,`idProducto`,`nomProducto`,`idTipoDescuento`,`nomDescuento`,`fechaInicio`,`fechaFin`,`estado`) values (1,'PRODUCTO','4','CHOMPA DE LANA',2,'DESCUENTO DE 10%','2015-09-07','2015-11-12',0),(2,'CATEGORIA','1','SIN CATEGORIA',3,'DESCUENTO 02','2015-09-07','2015-09-29',0),(3,'PROVEEDOR','45454545454','CUSCO HOSTING',1,'DESCUENTO NAVIDEÃ‘O','2015-09-07','2015-09-29',0);
...
insert  into `proveedor`(`documento`,`razon`,`telefono`,`direccion`,`fechaRegistro`,`email`,`estado`) values ('12222333333','WERWREWREW','2343','RERRR','2015-11-04','rwerwr',0),('20490253867','ITDECSA','245844','AV LA CULTURA NÂ° 772','2015-07-01','informes@itdecsa.com',0),('45454545454','CUSCO HOSTING','44444444','CUSCO PERU','2015-07-01','cuscohos@hotmail.com',0);
...
insert  into `unidades`(`id`,`nombre`,`abreviatura`,`valorBase`,`estado`) values (1,'UNIDADES','UNI',1,0),(2,'CAJAS','CAJ',1,0),(3,'BOLSAS','BOL',1,0),(4,'TABLETAS','TAB',1,0);
...
BEGIN
	...
	THEN 
		...
		FROM producto
		WHERE (id LIKE CONCAT('%',pFiltro,'%') OR nombre LIKE CONCAT('%',pFiltro,'%')) AND estado=0  
		ORDER BY nombre;
	...
	THEN 
		...
		FROM producto
   		WHERE (nroserie LIKE CONCAT('%',pFiltro,'%') OR nombre LIKE CONCAT('%',pFiltro,'%'))  AND estado=0  
		ORDER BY nombre;
	...
	THEN 
		...
		FROM proveedor
		WHERE (documento LIKE CONCAT('%',pFiltro,'%') OR razon LIKE CONCAT('%',pFiltro,'%')) AND estado=0  
		ORDER BY razon;
	...
	THEN 
		SELECT id, dimencion    
		FROM caracfisicas
		WHERE (id LIKE CONCAT('%',pFiltro,'%') OR dimencion LIKE CONCAT('%',pFiltro,'%')) AND estado=0  
		ORDER BY dimencion;
...
/* Procedure structure for procedure `producto_unidad` */

/*!50003 DROP PROCEDURE IF EXISTS  `producto_unidad` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `producto_unidad`(pIdUnidad int(8))
BEGIN
    
    SELECT `abreviatura` from `unidades` where `id`=pIdUnidad;
...
BEGIN
    ...
    IF(pContador=1)
    THEN
	UPDATE proveedor SET `razon`=pRazon,`telefono`=pTelefono,`direccion`=pDireccion,
	`fechaRegistro`=pFechaRegistro,`email`=pEmail,`estado`=pEstado WHERE `documento`=pDocumento;
    ELSE   
           insert INTO proveedor VALUES(pDocumento,pRazon,pTelefono,pDireccion,pFechaRegistro,pEmail, pEstado);
    END IF; 
...
BEGIN
    ...
    SELECT privilegio,concat(nombre,' ',apaterno,' ',amaterno),dni
    FROM usuario
...
/* Procedure structure for procedure `ventas_guarda` */

/*!50003 DROP PROCEDURE IF EXISTS  `ventas_guarda` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `ventas_guarda`(pId INT(10),pIdCliente CHAR(11),pSubtotal decimal(8,2),pImpuesto DECIMAL(8,2),
 pTotal decimal(8,2),pIdFormaPago int(1),pIdUsuario int(8),pFechaVenta date, pSerie char(20),pNroVenta int(8),pNroCaja int(8),pIdTipoComprobante int(8),pEstado int(1))
BEGIN
    ...
    /*los datos de una venta no se modifican, solo el estado, que se realizara en otro porcedimiento que no recepcione muchos parametros*/
           INSERT INTO ventas VALUES(pId,pIdCliente,pSubtotal,pImpuesto,pTotal,pIdFormaPago,pIdUsuario,pFechaVenta,pSerie,pNroVenta,pNroCaja,pIdTipoComprobante,pEstado);
    	
    END */$$
DELIMITER ;

/* Procedure structure for procedure `ventas_id` */

/*!50003 DROP PROCEDURE IF EXISTS  `ventas_id` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `ventas_id`()
BEGIN
...
/* Procedure structure for procedure `ventaxproducto_guarda` */

/*!50003 DROP PROCEDURE IF EXISTS  `ventaxproducto_guarda` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `ventaxproducto_guarda`(pId int(8),pIdVenta int(8),pIdProducto INT(8),pNomProducto varchar(250),pUnidad char(10),pCantidad int(8),pPrecioUnitario DECIMAL(10,2), pPrecioTotal DECIMAL(10,2))
BEGIN
	...
	INSERT INTO `ventasxproducto` (`id`,`idVentas`,`idProducto`,`nomProducto`,`unidad`,`cantidad`,`precioUnitario`,`precioTotal`) VALUES(pId,pIdVenta,pIdProducto,pNomProducto,pUnidad,pCantidad,pPrecioUnitario,pPrecioTotal);
    	
    		
END */$$
DELIMITER ;

/* Procedure structure for procedure `venta_buscaDescuento` */

/*!50003 DROP PROCEDURE IF EXISTS  `venta_buscaDescuento` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `venta_buscaDescuento`(pIdProducto int(8), pCriterio char(50), pFecha date)
BEGIN
     
	DECLARE idDescuento char(11);
	DECLARE fechaIni date;
	declare fechaf date;
	declare idTipoDes int(8);
	declare pPorcentaje int(8);
	declare diferencia1 int(8);
	declare diferencia2 int(8);
	DECLARE diferencia3 INT(8);
	
    
	/*VERIFICACION 01 --> SI EL DESCUENTO ES POR CATEGORIA*/
	IF(pCriterio="CATEGORIA")
	THEN
		SELECT `idCategoria` into idDescuento FROM `producto` WHERE `id`= pIdProducto;
	END IF;
	
	/*VERIFICACION 02 --> SI EL DESCUENTO ES POR PROVEEDOR*/	
	IF(pCriterio="PROVEEDOR")
	THEN
		SELECT  pro.`documento` INTO idDescuento FROM `compraxproducto` comp,`compras` com,`proveedor` pro WHERE pro.documento=com.idProveedor AND com.id=comp.idCompra AND comp.idProducto=pIdProducto;
	END IF;
	/*VERIFICACION 03 --> SI EL DESCUENTO ES POR PRODUCTO*/
	IF(pCriterio="PRODUCTO")
	THEN
		SET idDescuento= pIdProducto;
	END IF;
	
	/*---------OBTENEMOS EL ID DEL DESCUENTO DESPUES DE DETERMINAR SI EL DESCUENTO ESTA EN EL RANGO DE FECHAS ----*/
	SELECT idTipoDescuento, `fechaInicio`,`fechaFin` into idTipoDes, fechaini, fechaf FROM descuento WHERE `idProducto`= idDescuento AND `criterio`=pCriterio;
	
	/*---AQUI REALIZAMOS UNA DIFERENCIA DE FECHAS PARA SABER SI ESTA DENTRO DEL INTEVALO ----*/
	SET diferencia1= DATEDIFF(pFecha,fechaini);
	SET diferencia2= DATEDIFF(fechaf,pFecha);
	SET diferencia3= DATEDIFF(fechaf,fechaini);
	
	/*POSITIVISANDO LAS DIFERENCIAS*/
	SET diferencia1= SIGN(diferencia1)*diferencia1;
	SET diferencia2= SIGN(diferencia2)*diferencia2;
	SET diferencia3= SIGN(diferencia3)*diferencia3;
	
	/*---- los resultados deben ser mayores que 0 para que cumpla la condicion----*/
	if((diferencia1+diferencia2)=diferencia3)
	then
		/*----AQUI OBTENEMOS EL PORCENTAJE DE DESCUENTO SI ES QUE HAY ---*/
		SELECT porcentaje into pPorcentaje FROM `tipodescuento` WHERE `id`= idTipoDes;	
	else 
		set pPorcentaje=0;
	end if;
...
BEGIN
    ...
    DECLARE pContador INT(10);
    DECLARE ultimoNro CHAR(8);
    DECLARE ultimoSerie CHAR(8);
    declare pId int(8);
    ...
    select ultimoSerie, ultimoNro+1; 
