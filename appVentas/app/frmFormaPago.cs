...
namespace app
{
    public partial class frmFormaPago : Form
    {
        ...
        object[] DatosFormas = new object[3];
        string string_ArchivoConfiguracion;
        public frmFormaPago(string ArchivoCOnfig)
        {
            InitializeComponent();
        ...
        private void listar()
        {
            ...
            DataSet datos = ConexionBD.EjecutarProcedimientoReturnDataSet("formaspago_lista");
        ...
        {
            if (DatosFormas == null || DatosFormas[0] == null)
            {
                DatosFormas = new object[3];
                DatosFormas[0] = "0";
            }
            DatosFormas[0] = DatosFormas[0].ToString().Trim();
            DatosFormas[1] = tbNombre.Text;
            ...
            object[] NombresFormas = { "pId", "pNombre", "pEstado" };

            ConexionBD.Conectar(true, string_ArchivoConfiguracion);
            bool SeGuardo = false;
            try
            {
                ConexionBD.EjecutarProcedimientoReturnVoid("formaspago_guarda", NombresFormas, DatosFormas);
                ConexionBD.COMMIT();
                SeGuardo = true;
                listar();
                tipo = Tipo.guardar;
        ...
        private void dgvDatos_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if ((e.RowIndex >= 0) && ((e.ColumnIndex == 0) || (e.ColumnIndex == 1)))
            {
                ...
                {// EDITAR
                    ...
                    DatosFormas= new object[3];
                    DatosFormas[0] = dgvDatos.Rows[e.RowIndex].Cells[2].Value.ToString();
                ...
                {//ELIMINAR
                    if (MessageBox.Show("¿ESTA SEGURO DE ELIMINAR ESTA FORMA DE PAGO? \r", "CUIDADO!!!", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Warning) == DialogResult.Yes)
                    {
                        //LimpiarDatosHorario();
                        ConexionBD.Conectar(true, string_ArchivoConfiguracion);
                        ...
                        try
                        {
                            ConexionBD.EjecutarProcedimientoReturnVoid("formaspago_elimina", "pId", dgvDatos.Rows[e.RowIndex].Cells[2].Value.ToString());
                            ConexionBD.COMMIT();
                            SeElimino = true;
                            listar();
                            tipo = Tipo.eliminar;
