...
BEGIN
	...
	SELECT idTipoDescuento, `fechaInicio`,`fechaFin` into idTipoDes, fechaini, fechafin FROM descuento WHERE `idProducto`= idDescuento AND `criterio`=pCriterio;
	
	/*FALTA COMPARAR FECHAS*/
	
	/*----AQUI OBTENEMOS EL PORCENTAJE DE DESCUENTO SI ES QUE HAY ---*/
	SELECT porcentaje FROM `tipodescuento` WHERE `id`= idTipoDes;
